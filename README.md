...
- 프레임 분석 결과는 `interest`(흥미도)와 `focused`(집중도)로 정리되어후속 로직으로 전달됩니다.

### 2) 음성 STT + LLM 요약
- 상담 음성은 `faster-whisper` 기반 STT로 텍스트화합니다.
- 변환된 텍스트는 OpenAI/Ollama 기반 요약 모듈로 전달되어 상담 핵심 요약과 추천 진로 후보를 도출합니다.
...
- 설문점수는 1점부터 5범을 0점 부터 100점으로 정규화합니다.
